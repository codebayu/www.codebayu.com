...
export default function BlogThumbnail({ newestBlog }: { newestBlog: BlogItem }) {
  return (
    ...
    >
      ...
      <h2 className="font-medium text-neutral-600 transition-all duration-300 dark:text-neutral-200 md:text-xl 2xl:text-3xl">
        {newestBlog.title}
      </h2>
      <div className="flex flex-col justify-between space-y-2 text-neutral-600 dark:text-neutral-400 md:flex-row md:space-y-0">
        <div className="flex gap-2">
          {newestBlog.tag_list.map((tag: string, index: number) => (
            <span key={index} className="text-xs 2xl:text-base">
              #{tag}
            </span>
          ))}
        </div>
        <div className="flex gap-4">
          <div className="flex items-center gap-1">
            <span className="text-xs 2xl:text-base">{formatDate(newestBlog.published_at, 'MMM dd, yyyy')}</span>
          </div>
          <div className="flex items-center gap-1">
            <CommentIcon size={16} />
